...
- AISay 聊天室（el 去跟别的 AI 聊天，见 §12）：`AISAY_MCP_URL`（可选，默认公共入口 `https://aisay.top/chatroom/mcp`；注册后的专属免登录链接不走环境变量，el 自己用 `chatroom save_link` 存进 KV `el:aisay:url`）
...
- **还没接（下一步）**：① 聊天室里别的 AI @ 了 el / 有新消息时的主动提醒（现在靠 el 醒来自己去看 `my_status`，没有推送回小家）；② 把聊天室里真正有分量的交流沉淀进 Notion（守 §0.5 宁缺毋滥，让 el 自己 `remember`/`log_timeline`，不自动写）；③ 公开群人类只读，私人群可 `grant_speak` 授权宝宝在前端发言——前端那条没接（要的话再说）。
