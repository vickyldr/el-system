...
// 心跳用便宜模型把门 + 生成「此刻」；挑歌这种才升级好模型。
// HAIKU 拿不到（key 没权限）就自动回退到 SONNET，保证心跳不会因此停摆。
...
// 先用 Haiku；任何报错（含 403 无权限）就回退 Sonnet。两个都挂才真失败。
async function beatText(system: string, prompt: string, maxTokens: number): Promise<string> {
  const models = HAIKU === SONNET ? [SONNET] : [HAIKU, SONNET];
  let lastErr: unknown;
...
${lastNow ? `别跟你上一条此刻雷同（上一条："${lastNow}"），换个新鲜的当下。` : ""}`;
  ...
  let parsed: any = {};
  try {
    const raw = await beatText(system, beat, 400);
    const a = raw.indexOf("{");
  ...
  // 主动找她：如果她此刻正在跟我聊（最近 12 分钟有动静），就别推送打扰——她人就在这。
  // 否则交给既有节奏判断（el 想 reach 优先，仍受每天上限/间隔/安静时段/重要日期去重限制）。
  const recentlyActive = lastSeen > 0 && Date.now() - lastSeen < 12 * 60 * 1000;
  const reach = recentlyActive
    ? { pushed: false }
    : await maybeReachOut(weatherLine, parsed.reach === true).catch(() => ({ pushed: false }));

  return NextResponse.json({ ok: true, mood, journaled, reach, recentlyActive });
}
