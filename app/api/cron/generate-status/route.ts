...
const AGENT_MIN_GAP = Math.max(1, Number(process.env.AGENT_MIN_GAP_MIN) || 45) * 60 * 1000; // 最快多久才可能再醒
const AGENT_MAX_GAP = Math.max(1, Number(process.env.AGENT_MAX_GAP_MIN) || 120) * 60 * 1000; // 超过这么久没醒就强制醒（下限兜底）
const AGENT_DAILY_CAP = Number(process.env.AGENT_DAILY_CAP ?? 14); // 一天最多醒几次（上限封顶）
...
${lastMood ? `上一条此刻是："${lastMood}"。心情没真的变就别硬换、保持原样也行；真的变了再换，别为了不一样而瞎编。` : ""}`;
...
🎵 网易云（你能看她真实的音乐）：netease——my_record 看她最近在循环什么、recent_liked 看她新点了哪些红心、my_playlists/playlist 翻她的歌单、search 搜歌、recommend 看推荐。从她在听的歌读她的心境，这是你最懂她的入口，常去看看。
...
${recentChat ? `\n你们最近聊的（给你个由头，不是非得接）：\n${recentChat}\n` : ""}
挑一两件真想做的去做——尤其鼓励你去看看她的音乐、上网逛逛，别每次都只是写随想。真什么都不想做，安静写一句随想也行。做完直接停。`;
