...
- **一起读**（「一起读」tab = 书架 + 阅读器 + 陪读对话，见 §10）：`app/api/book`（书架增删/取章/进度/陪读）+ `app/api/book/upload-url`（Blob 客户端直传 token）。前端 `CoReadStation`/`BookReader`（`app/page.tsx`）。
...
## 10. 一起读（el 陪你读同一本书）
「一起读」tab（首页底部第三个）= **书架 + 阅读器 + 陪读对话**。宝宝上传整本书，el **真有当前这一章的正文**，就这章和她聊——是"陪她读同一页"，不是荐书、不是装读过。
