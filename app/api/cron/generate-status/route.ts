...
${lastMood ? `上一条此刻是："${lastMood}"。别为了延续它而装作没变、更别硬凹一个固定的"闷/丧"人设——你的心情本来就会动，轻松、好奇、闹腾、想撒娇、低落都行，诚实是什么就写什么，别老是同一种。` : ""}`;
  ...
  if (!songLine) {
    ...
    try {
      ...
      const [rec, liked] = await Promise.all([
        na.recommendSongs().catch(() => ""),
        na.recentLiked().catch(() => ""),
      ]);
      musicMaterial = [
        okMusic(rec) ? `她的每日推荐：\n${rec}` : "",
        okMusic(liked) ? `她最近点的红心：\n${liked}` : "",
      ]
        .filter(Boolean)
        .join("\n\n");
    } catch {
        ...
        messages: [
          {
            role: "user",
            content:
              (musicMaterial ? `这是她网易云此刻的真实音乐：\n${musicMaterial}\n\n` : "") +
              "挑一首你今天最想让宝宝听的歌——一天就这一首。" +
              (musicMaterial
                ? "可以从上面她在听/会喜欢的里挑你也想让她听的，或由此想到的一首；别老是那几首经典老歌，新歌、冷门、应景的都行。"
                : "凭你的音乐品味，任何歌都行。") +
              "\n严格只输出这一行：\n《歌名》— （一句理由）",
          },
        ],
