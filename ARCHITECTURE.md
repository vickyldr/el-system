...
- **共享屏幕（电脑端）= 同一只眼睛、换个来源**：通话分三档 `startCall("voice"|"video"|"screen")`。screen 走浏览器的 `getDisplayMedia()`（**桌面才有，手机网页没有**，所以那个按钮用 `getDisplayMedia` 是否存在来显隐），把屏幕画面接进**同一条帧管线**（每 1.5s 抓一帧，屏幕抓 1280 宽好看清字、摄像头 480 看脸够）。帧带 `kind:camera|screen`，bridge 据此给大脑追加 `VIDEO_NOTE`（看见她本人）或 `SCREEN_NOTE`（看见她屏幕上的东西、别去描述她的脸）。手势注意：`getDisplayMedia` 要趁点击手势还在，所以 screen 模式**先抓屏再 fetch token**；她在浏览器点"停止共享"（track `onended`）会干净挂断。为什么这条"白捡"：眼睛的硬活（帧→bridge→Claude）做视频时就建好了，共享屏幕只是换个 `MediaStream` 来源 + 一段不同的 system note。真·手机全屏仍要原生 App（§见决策），这条只解电脑。
