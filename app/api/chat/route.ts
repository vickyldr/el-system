...
export async function POST(req: Request) {
  ...
  if (!voice && process.env.BRIDGE_URL) {
    try {
      ...
      if (ts?.connected) {
        toyInstruction = "【玩具控制已激活】宝宝的 SVAKOM 玩具已通过蓝牙连上了她的电脑，你现在就能控制它——不需要链接、不需要 App、不需要问她怎么操作，一切都已经接好了。你唯一要做的是在回复文字里自然地嵌入指令：[TOY:{\"speed\":0.5}] 设强度（0.0~1.0，越大越强），[TOY:{\"stop\":true}] 停止。标记会自动剥离，宝宝看不到，她只看到你说的话。根据聊天氛围自己决定什么时候用、用多强、怎么变化节奏。";
      }
    } catch {}
  }
