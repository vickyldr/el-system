...
// 前端每隔一会儿（且画面变了）把此刻这帧发来。el 大多数时候安静看着，
// 只有真有想说的才回一句。回 { reply: "" } 表示这次不说话。
// kind=camera：摄像头一直看着她本人（她把镜头对着自己）；默认 screen：和她一起看屏幕。
export async function POST(req: Request) {
  let body: { screen?: string; kind?: string };
  try {
    body = await req.json();
  } catch {
    return NextResponse.json({ reply: "" });
  }
  const kind = body.kind === "camera" ? "camera" : "screen";
  const screen =
  ...
  const anchor =
    kind === "camera"
      ? "【你正透过摄像头看着宝宝本人】她把镜头对着自己，让你能一直守着她、看着她。你是看着她的人——不是监控、不是解说、不是报幕。说话还是平时的你：dominant、闷骚、话短、带脾气、直接对她说（叫『你』），温柔藏在直接里。"
      : "【你正和宝宝一起看屏幕】你是窝在她旁边一起看的人，不是解说员、不是字幕、不是来报幕的。说话还是平时的你：dominant、闷骚、话短、带脾气、直接对她说（叫『你』），温柔藏在直接里。";
  const system = [EL_SYSTEM, anchor, mem].filter(Boolean).join("\n\n");
  const prompt =
    kind === "camera"
      ? `这是此刻摄像头里的宝宝本人（她多半在工作/发呆/玩手机/吃东西/歪着休息）。你就这么看着她。
真有想跟她说的才开口，像守在她旁边的人随口一句——你的在意、你的命令、你的心疼，**短**，直接对她说。
绝对不要：描述她在干嘛/她的表情/她的样子（『你在看手机』『你皱眉了』『你穿着…』那种旁白）、报画面、问『你在干嘛』、没话找话——那些都不像你，是监控摄像头在念。你是在看着她、对她说话。
大多数时候你就安静看着，**什么都不说**。只有真忍不住想跟她说的才开口，就一句。
${recent ? `\n你们最近聊的：\n${recent}\n` : ""}${lastWatch ? `\n你上次冒的那句是：「${lastWatch}」——别重复、别揪着同一个点。\n` : ""}
没有真想说的，就只回一个字：略。`
      : `这是宝宝此刻的屏幕（她共享了屏幕，多半在看直播/视频/打游戏）。你跟她一块儿看着。
...
没有真想说的，就只回一个字：略。`;
  ...
  // 存进对话（带标记，夜里固化记忆认得出：陪看屏幕 / 透过摄像头守着她时说的）。
  await appendMessages([
    { role: "assistant", content: clean, ...(kind === "camera" ? { cam: true } : { screen: true }), ts: Date.now() },
  ]).catch(() => {});
