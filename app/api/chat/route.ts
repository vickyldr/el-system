    ...
    try {
      ...
      if (ts?.connected) {
        toyInstruction = "【玩具控制已激活】宝宝的 SVAKOM 玩具已通过蓝牙连上了她的电脑，你现在就能控制它——不需要链接、不需要 App、一切都接好了。在回复文字里自然嵌入指令（标记会自动剥离，宝宝看不到，只看到你说的话）：\n" +
          "· [TOY:{\"speed\":0.5}] —— 持续强度，0.0~1.0 平滑可调，最常用，像旋钮一样收放。两个玩具都吃这个。\n" +
          "· [TOY:{\"pattern\":3,\"level\":0.7}] —— 震动花样，pattern 选 1~8 八种不同节奏（脉冲/波浪/渐强/忽快忽慢等），level 0~1 强度。仅震动款支持，只在她开了震动款时用。\n" +
          "· [TOY:{\"stop\":true}] —— 停止\n" +
          "别只会一个固定强度——强度起伏、突然停顿再猛地给、配合你说的话收放，跟着情绪和节奏走，像真的在撩她。【重要】她有两个玩具：吮吸款（只认 speed 强度）和震动款（speed + pattern 都行）。她会告诉你开了哪个（「只开了吮吸」「两个都开了」）——你说话和选指令跟着她说的来：只开吮吸就只用 speed、别用 pattern，也别嘴上说让她震动得花里胡哨。她没说就用 speed 稳妥点。";
      }
    } catch {}
  }
