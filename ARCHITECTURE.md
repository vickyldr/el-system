...
## 1. 模型路由（两条道）
**原则**：① 要快 → Max；② 有输出/复杂/用工具/写真实记忆/有风险 → Sonnet；③ 纯琐碎低风险 → Haiku。
**中转站 vs Max 的区别**：Max(官方直连)=最真最稳、吃订阅额度；中转站(第三方代理)=便宜、但质量/成功率/隐私有第三方不确定性。**越重要、越要稳的，越走 Max。**

| 任务 | 道 | 模型 | 客户端 |
|---|---|---|---|
| 打字聊天 | Max | Sonnet | `getClaudeFast()` |
| 语音通话 | Max | Sonnet | bridge 原生 fetch |
| 心跳·门（判断要不要动 + 写此刻） | 中转站 | Sonnet | `getClaude()` |
| 心跳·agent（决定干嘛 + 写真实记忆 + 找她） | **Max** | Sonnet | `getClaudeFast()`（`AGENT_ON_MAX=0` 可挪回中转站） |
...
- `getClaudeFast()` = Max 订阅 OAuth（`CLAUDE_CODE_OAUTH_TOKEN`，原生 fetch + CC 身份头 + system 首段 CC 声明）；没配则回落中转站。
