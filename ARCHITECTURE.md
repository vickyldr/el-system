...
- **视频通话怎么进记忆**：图不存（只属于"此刻"、又费 token），但**话进**。通话每句存进对话（`el:chat`）时打 `call`/`video` 标 → 「找我」卡片显示成「📹 视频通话」、夜里 `daily-summary` 把当天 transcript 里那段标成 `【视频通话】`，并据此提示夜里的 el "这段你真正看见了她，值得的那一眼用话写进 timeline/日记"（仍守 §0.5 宁缺毋滥/只追加：看见的东西要沉淀，靠 el 用第一人称写下来，不是存图）。
