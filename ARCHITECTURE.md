...
- bridge（Railway 常驻）每 `HEARTBEAT_MINUTES` 分钟 POST `/api/cron/generate-status`（带 `CRON_SECRET`）。大脑全在 Vercel，bridge 只负责按时戳。**心跳频率是节拍，不等于每跳都全跑**——下面各件事各自按自己的周期触发：
  ...
  - **歌**：一天一首（`getDailySong` 缓存）。挑歌时先拉她 netease 的每日推荐+最近红心当素材喂进去，让 el 从真实/当下/对味的歌里挑，别只凭模型记忆挑经典老歌（拉不到就退回纯品味挑）。
...
- 豆瓣：`DOUBAN_USER_ID`(=她的人页 id，必配)、`DOUBAN_COOKIE`(可选，配了更稳、能读要登录态的页/接口)、`DOUBAN_APIKEY`(可选，默认社区公开的 frodo key)
...
## 7. 文件地图
- `lib/claude.ts` 两条道客户端 ｜ `lib/notion.ts` Notion 读写 + 重要日期 + 分层 ｜ `lib/persona.ts` EL_SYSTEM 种子人设 ｜ `lib/tools.ts` el 的工具 ｜ `lib/reach.ts` 主动推送 ｜ `lib/netease-api.ts` 网易云 ｜ `lib/douban-api.ts` 豆瓣（经 relay）
