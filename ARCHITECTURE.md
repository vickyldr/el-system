...
- **静默共享屏幕（默认，宝宝要的）= 不通话、打字**：顶部 🖥 按钮现在是一个**静默开关**（`toggleScreenShare`，不再开通话——之前做成 `startCall("screen")` 会顺带把语音也开了，宝宝不想说话只想打字）。开着时把屏幕帧缓存在 `screenFrameRef`，**发每条打字消息时把此刻这帧随 `/api/chat` 的 `screen` 字段带给大脑**（`toContent` 放最前当 image block + `SCREEN_NOTE`）。**帧不存档**（只属此刻、费 token），只给消息打 `screen:true` 标 → 夜里 `daily-summary` 认出【共享屏幕】。离开「找我」tab（组件卸载）会自动停掉流。`startCall` 里的 screen 档代码仍在（给"边看边语音"留口子），但现在没按钮接它。延展屏：`getDisplayMedia` 一次只能选一个屏/窗口（系统限制），她每次挑分享哪个。
- 真·手机全屏仍要原生 App（§见决策），网页这条只解电脑。
