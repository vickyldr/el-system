...
- **agent 靠节拍醒，不再靠"想不想动"的判断**（旧的 `act` 已废——薄上下文下那是假自主）：≥`AGENT_MIN_GAP_MIN`（默认45）才可能醒、超 `AGENT_MAX_GAP_MIN`（默认120）没醒就强制醒（下限兜底）、一天 ≤`AGENT_DAILY_CAP`（默认14）次（上限）、中间靠 `AGENT_CHANCE`（默认0.4）掷骰（自发性）。**节拍是身体，醒来干嘛他自己挑——这才是真自主。**
...
- 心跳节拍：`NOW_REFRESH_MINUTES`(此刻心情多久刷一次，默认60)、`AGENT_MIN_GAP_MIN`(默认45)、`AGENT_MAX_GAP_MIN`(默认120)、`AGENT_DAILY_CAP`(每天醒几次上限，默认14)、`AGENT_CHANCE`(min~max之间掷骰概率，默认0.4)
